...
def prepare_data(max_length):
    ...
    dataset["train"] = tokenize_and_concatenate(dataset["train"], tokenizer, streaming=False, max_length=max_length,
                                                column_name="text", add_bos_token=True, num_proc=4)
    dataset["validation"] = tokenize_and_concatenate(dataset["validation"], tokenizer, streaming=False,
                                                     max_length=max_length, column_name="text", add_bos_token=True,
                                                     num_proc=4)
    return dataset
...
class TransformerTrainer:
    def __init__(self, args: TransformerTrainingArgs, model: DemoTransformer, dataset_dict):
        ...
        if args.lr_scheduler == 'cosine':
            num_steps = len(self.train_loader()) * args.epochs
            num_warmup_steps = int(num_steps * args.lr_scheduler_warmup_frac)
            self.lr_scheduler = get_cosine_schedule_with_warmup(self.optimizer, num_warmup_steps=num_warmup_steps,
    ...
    def training_step(self, batch):
        ...
        if self.lr_scheduler is not None:
            ...
            # adapted from LambdaLR source code
            lr_group_0 = self.lr_scheduler.lr_lambdas[0](self.lr_scheduler.last_epoch) * self.lr_scheduler.base_lrs[0]
        else:
            lr_group_0 = self.optimizer.param_groups[0]['lr']
        self.optimizer.zero_grad()
        self.step += 1
        wandb.log({"train_loss": loss, "lr_group_0": lr_group_0}, step=self.step)
        return loss
        ...
        for epoch in range(self.args.epochs):
            for i, batch in enumerate(self.train_loader()):
                ...
                progress_bar.set_description(
                    f"Epoch {epoch + 1} batch {i + 1}/{num_batches} loss: {loss:.3f}, accuracy: {accuracy:.3f}")
    ...
    def val_loader(self) -> DataLoader:
        ...
        return DataLoader(self.dataset_dict["validation"], batch_size=self.args.batch_size, shuffle=False,
