...
for lr in "${muP_learning_rates[@]}"; do
  ...
  for width in "${widths[@]}"; do
    ...
    python train.py --lr $lr --width $width --wandb_project $wandb_project --wandb_run_prefix $wandb_run_prefix --apply_muP --lr_scheduler cosine --fix_layernorm --fix_embed_lr --fix_unembed
  done
done